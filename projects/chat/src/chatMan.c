 ...
 */   
     sprintf(command, "gnome-terminal sh --command \"./wr.out %s %d %s %s\"", _ipAdd, _port, FILE_OF_PID, _userName);
     system(command);
    while(fp==NULL)
    {
 ...
 */    
     sprintf(command, "gnome-terminal sh --command \"./re.out  %s %d %s %s\"", _ipAdd, _port, FILE_OF_PID, _userName);
    system(command);
    fp=NULL;
     while(fp==NULL)
    {
